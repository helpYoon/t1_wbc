...
# The golden fixture was captured under the original task weights; pin them here so this test
# keeps validating the assembly MATH (numpy == torch oracle) independent of the live defaults,
# which were re-tuned for hardware (w_base_ori, w_post).
GOLDEN_CFG = WBCConfig(w_base_ori=5.0, w_post=1.0)
...
def test_numpy_assemble_matches_torch_golden():
    ...
    qp = assemble_wbc_qp(d, _targets(), GOLDEN_CFG, F["ctrlrange"], nv, nu)
